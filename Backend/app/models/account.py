from sqlalchemy import Column, DECIMAL, String, TIMESTAMP, ForeignKey, ForeignKeyConstraint, func
...
class Account(Base):
    ...
    party_type = Column(ENUM("S", "B", "CA"), nullable=False)

    transaction_type = Column(ENUM("payment", "refund", "commission"), nullable=False)
    ...
    payment_id = Column(INTEGER(unsigned=True), ForeignKey("payments.payment_id"), nullable=True)
    ...
    def __repr__(self):
        return f"<Account account_id={self.account_id} transaction_type={self.transaction_type}>"
